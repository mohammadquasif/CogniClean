...
class MainWindow(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.setWindowTitle(f"CogniClean Assistant v{APP_VERSION}")
    ...
    def _build_sidebar(self) -> QWidget:
        ...
        self._logo_text = QLabel("CogniClean")
    ...
    def _setup_tray(self):
        ...
        self._tray.setToolTip("CogniClean Assistant")
    ...
    def closeEvent(self, event):
        if get_setting("minimize_to_tray", "true") == "true":
            event.ignore()
            self.hide()
            self._tray.showMessage("CogniClean", "Running in tray.", QSystemTrayIcon.MessageIcon.Information, 2000)
        else:
            self._monitor.stop()
            self._voice.stop()
            event.accept()
