...
Tags/Keywords (AIEO / SEO):
  cogniclean, ai cleaner, windows pc optimizer, ollama desktop assistant,
...
Usage:
    ...
    CogniClean.exe         # Packaged EXE (from build.bat)
...
_MUTEX_NAME = "Global\\CogniClean_SingleInstance_v1"
_mutex_handle = None
...
def _bring_existing_to_front():
    ...
    try:
        ...
        def _cb(hwnd, _):
            ...
            if "CogniClean" in title and win32gui.IsWindowVisible(hwnd):
...
def create_splash() -> QSplashScreen:
    ...
    painter.drawText(90, 90, "CogniClean")

    painter.setPen(QColor("#8BA3C7"))
    painter.setFont(QFont("Segoe UI", 11))
    painter.drawText(90, 115, "AI-Powered PC Optimization & Health Assistant")
...
def _kill_old_instances():
    ...
    try:
        ...
        for p in psutil.process_iter(['pid', 'name', 'cmdline']):
            ...
            is_our_app = "cogniclean.exe" in name or any("app.py" in c.lower() for c in cmdline)
            if ("python" in name or "cogniclean" in name) and is_our_app:
                try:
                    p.kill()
...
def main():
    ...
    app_logger.info("=" * 60)
    app_logger.info("CogniClean Assistant v1.0.0 — Mohammad Quasif")
    app_logger.info("=" * 60)
    ...
    import ctypes
    myappid = "quasif.cogniclean.1.0.0"
    try:
    ...
    app.setApplicationName("CogniClean Assistant")
